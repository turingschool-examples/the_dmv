class FacilityFactory
  ...
  def create_mo_facilities(facilities)
    facilities.map do |facility|
      formatted_phone = facility[:phone].delete(" ()").insert(3, "-") unless facility[:phone] == nil
      formatted_mo_address = facility[:address1] + ", " + facility[:city] + ", " + facility[:state] + ", " + facility[:zipcode]
      facility_details = {}
      facility_details[:name] = facility[:name]
      facility_details[:phone] = formatted_phone
      facility_details[:address] = formatted_mo_address
      Facility.new(facility_details)
    end
  end
end