...
RSpec.describe Analytics do
    ...
    it "brings registered vehicles array when instantiated" do  
      expect(@analytics.facility.registered_vehicles).to eq ([@cruz, @camaro, @bolt, @tesla_1, @tesla_2])      
    end
    end

    describe "Find the most popular make/model registered" do
    it "can populate the ev vehicles registered" do   
      expect(@analytics.ev_vehicles_registered).to eq ([@bolt, @tesla_1, @tesla_2])
    end

    it "can populate the registered ev vehicle makes" do
      expect(@analytics.reg_ev_vehicles_makes).to eq (["Chevrolet", "Tesla", "Tesla"])
    end
