...
RSpec.describe DataReview do
  before(:each) do
    ...
    @facility_1.first.register_vehicle(@vehicles[3])
    @research = DataReview.new(@facility_1[0])
  end 

  it 'can create a new research variable that is analyzing one facility' do
    expect(@research.class).to eq(DataReview)
  end

  it 'can find the popular make by facility' do
    expect(@research.find_popular_by_make).to eq("Tesla is the most popular make at the DMV Tremont Branch")
  end

  it 'can find the popular model by facility' do
  expect(@research.find_popular_by_model).to eq("Model x is the most popular make at the DMV Tremont Branch")
  end

  it 'can return the registered vehicle count by given model year' do
  expect(@research.registered_vehicle_count_by_model_year(2015)).to eq("1 registered vehicle(s) per the model year of 2015 at the DMV Tremont Branch")
  end
