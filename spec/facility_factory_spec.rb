...
RSpec.describe FacilityFactory do
    ...
    it "creates a collection with no nil or empty values" do
        expect(@factory.create_facilities(@ny_dmv_locations)).to be_an_instance_of (Array)
        ...
        expect(@factory.facilities.first).to eq(@factory.create_facilities(@ny_dmv_locations).first)
    end
    it "assigns the attributes correctly" do
        ...
        expect(@factory.create_facilities(@ny_dmv_locations).first.services).to eq (@ny_dmv_locations.first[:services])
    end
    end
end