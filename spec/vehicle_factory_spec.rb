...
RSpec.describe do
    ...
    it "#wa_create_vehicles" do
        ...
        expect(factory.wa_create_vehicles(wa_ev_registrations).first.engine).to eq(:ev)
        expect(factory.wa_create_vehicles(wa_ev_registrations).first.registration_date).to eq(nil)
        expect(factory.wa_create_vehicles(wa_ev_registrations).first.plate_type).to eq(nil)
        expect(factory.wa_create_vehicles(wa_ev_registrations).first.vin).not_to be(nil)
        expect(factory.wa_create_vehicles(wa_ev_registrations).first.year).not_to be(nil)
        expect(factory.wa_create_vehicles(wa_ev_registrations).first.make).not_to be(nil)
        expect(factory.wa_create_vehicles(wa_ev_registrations).first.model).not_to be(nil)
    end
    ...
    it "#most_popular_make" do
        ...
        vehicles = vehicle_factory.wa_create_vehicles(ev_source_array)
